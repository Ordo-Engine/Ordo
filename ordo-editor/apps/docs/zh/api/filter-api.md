...
| `format`         | `"sql"` \| `"json"` | —    | 输出格式。默认：`"sql"`。                                                                           |
| `field_mapping`  | object              | —    | 将规则字段路径映射到数据库列名。未映射的字段默认将 `.` 替换为 `_`。                                 |
| `max_paths`      | number              | —    | 收集的最大路径数，超出后停止。默认：`100`。                                                         |
...
| 字段             | 类型                     | 说明                                                                                       |
| ---------------- | ------------------------ | ------------------------------------------------------------------------------------------ |
| `filter`         | string \| object \| null | 生成的过滤条件。SQL 格式为字符串，JSON 格式为对象，`never_matches` 为 true 时返回 `null`。 |
...
字符串字面量使用单引号转义（`'` → `''`）。算术运算符和不支持的函数将返回 `500` 错误。
