...
export function useRolloutStrategyLabel() {
  ...
  return (strategy: RolloutStrategy) => {
    switch (strategy.kind) {
      ...
      case 'fixed_batch':
        return t('releaseCenter.rolloutStrategyLabel.fixedBatch', { count: strategy.batch_size ?? 0 })
      case 'percentage_batch':
        return t('releaseCenter.rolloutStrategyLabel.percentageBatch', { pct: strategy.batch_percentage ?? 0 })
