      ...
      if (ex && !isLiveExecution.value) {
        ...
        if (ex.started_at)
          elapsedSeconds.value = Math.floor(
            (Date.now() - new Date(ex.started_at).getTime()) / 1000
          );
...
async function executeRelease() {
  ...
  try {
    ...
    startPolling();
    MessagePlugin.success(
      t(isRetry ? 'releaseCenter.executionRetried' : 'releaseCenter.executionStarted')
    );
  } catch (e: any) {
...
async function controlExecution(action: 'pause' | 'resume' | 'rollback') {
  ...
  try {
    const ex =
      action === 'pause'
        ? await releaseApi.pauseExecution(
            auth.token,
            route.params.orgId as string,
            route.params.projectId as string,
            request.value.id
        ...
        : action === 'resume'
          ? await releaseApi.resumeExecution(
              auth.token,
              route.params.orgId as string,
              route.params.projectId as string,
              request.value.id
            )
          : await releaseApi.rollbackExecution(
              auth.token,
              route.params.orgId as string,
              route.params.projectId as string,
              request.value.id
...
<template>
  <div class="view-page">
    <StudioPageHeader
      ...
      :subtitle="
        request
          ? `${request.ruleset_name} · ${request.environment_name} · v${request.version}`
          : t('releaseCenter.requestDetailSubtitle')
      "
    >
      <template #actions>
        <t-button
          variant="outline"
          @click="router.push({ name: 'project-release-requests', params: route.params })"
        >
      ...
      <t-card :bordered="false" class="strip-card">
        <div class="strip">
          ...
          <div class="strip-item">
            ...
            <strong>{{
              request.version_diff.rollback_version || request.rollback_version || '—'
            }}</strong>
          </div>
          <div class="strip-item">
            <span>{{ t('releaseCenter.rolloutStrategy') }}</span>
            <strong>{{
              labelRolloutStrategy(
                request.request_snapshot.rollout_strategy || request.rollout_strategy
              )
            }}</strong>
          </div>
          <div class="strip-divider" />
      ...
      <t-tabs v-model="activeTab" class="detail-tabs">
          ...
          <div class="panel">
              ...
              <div class="kv-grid kv-grid--3">
                <div class="kv">
                  ...
                  <strong>{{
                    ...
                    request.request_snapshot.requester_name ||
                    request.created_by
                  }}</strong>
                </div>
                <div class="kv">
                  ...
                  <strong>{{
                    request.created_by_email || request.request_snapshot.requester_email || '—'
                  }}</strong>
                </div>
                <div class="kv">
                ...
                <div class="kv">
                  ...
                  <strong>{{
                    request.request_snapshot.affected_instance_count ??
                    request.affected_instance_count
                  }}</strong>
                </div>
                <div class="kv">
                  <span>{{ t('releaseCenter.environmentField') }}</span>
                  <strong>{{
                    request.request_snapshot.environment_name || request.environment_name || '—'
                  }}</strong>
                </div>
                <div class="kv">
                  <span>{{ t('releaseCenter.createdAt') }}</span>
            ...
            <t-card v-if="request.change_summary || request.release_note" :bordered="false">
              <div class="text-sections">
                ...
                <div
                  v-if="request.release_note"
                  class="text-block"
                  :class="{ 'text-block--bordered': request.change_summary }"
                >
            ...
            <div class="two-col">
              <t-card :bordered="false">
                <div class="card-section-title">
                  {{ t('releaseCenter.rolloutStrategyDetails') }}
                </div>
                <div class="kv-grid kv-grid--2">
                  ...
                  <div
                    v-if="request.request_snapshot.rollout_strategy?.batch_interval_seconds"
                    class="kv"
                  >
                    <span>{{ t('releaseCenter.batchIntervalField') }}</span>
                    <strong
                      >{{
                        request.request_snapshot.rollout_strategy.batch_interval_seconds
                      }}s</strong
                    >
                  </div>
                  <div
                    v-if="request.request_snapshot.rollout_strategy?.pause_on_error_rate"
                    class="kv"
                  >
                    ...
                    <strong
                      >{{
                        (
                          request.request_snapshot.rollout_strategy.pause_on_error_rate * 100
                        ).toFixed(0)
                      }}%</strong
                    >
                  </div>
              ...
              <t-card :bordered="false">
                ...
                <div class="kv-grid kv-grid--2">
                  <div class="kv">
                    ...
                    <strong>{{
                      request.request_snapshot.rollback_policy?.auto_rollback
                        ? t('common.enabled')
                        : t('common.disabled')
                    }}</strong>
                  </div>
                  <div class="kv">
                    ...
                    <strong>{{
                      request.request_snapshot.rollback_policy?.max_failed_instances ?? '—'
                    }}</strong>
                  </div>
        ...
        <t-tab-panel value="approval" :label="t('releaseCenter.tabApproval')">
          <div class="panel">
            ...
            <t-card :bordered="false" class="approval-summary-card">
              <div class="approval-summary">
                <div class="approval-progress-text">
                  ...
                  <span class="approval-total">{{
                    approvalProgress.min || approvalProgress.total
                  }}</span>
                  <span class="approval-label">{{ t('releaseCenter.approvalsRequired') }}</span>
                </div>
                <t-progress
                  :percentage="
                    approvalProgress.min
                      ? Math.min(
                          100,
                          Math.round((approvalProgress.approved / approvalProgress.min) * 100)
                        )
                  ...
                  "
                  :status="
                    requestDisplayStatus === 'approved' || requestDisplayStatus === 'completed'
                      ? 'success'
                      : requestDisplayStatus === 'rejected' || requestDisplayStatus === 'failed'
                        ? 'error'
                        : undefined
                  "
                  theme="line"
                ...
                <div class="approval-meta-pills">
                  <span class="meta-pill">
                    {{ t('releaseCenter.minApprovals') }}:
                    <strong>{{
                      request.request_snapshot.min_approvals ?? approvalProgress.total
                    }}</strong>
                  </span>
                  <span class="meta-pill">
                    {{ t('releaseCenter.policyField') }}:
                    <strong>{{ request.request_snapshot.policy_name || '—' }}</strong>
                  </span>
                </div>
              </div>
            ...
            <!-- My pending action -->
            <t-card
              v-if="canReview && request.status === 'pending_approval'"
              :bordered="false"
              class="my-action-card"
            >
              <div class="my-action">
                  ...
                  <div class="approval-body">
                    <div class="approval-head">
                      <div class="approval-who">
                        ...
                        <t-tag
                          v-if="approval.reviewer_id === auth.user?.id"
                          size="small"
                          variant="light"
                          theme="primary"
                        >
                      ...
                      <div class="approval-right">
                        <t-tag
                          size="small"
                          variant="light"
                          :theme="approvalDecisionTheme(approval.decision)"
                        >
                          {{ t(`releaseCenter.approvalMap.${approval.decision}`) }}
                        </t-tag>
                        <span v-if="approval.decided_at" class="approval-time">{{
        ...
        <t-tab-panel value="diff" :label="t('releaseCenter.tabDiff')">
          <div class="panel">
              ...
              <div class="diff-header">
                <div class="diff-stat-row">
                  <div class="diff-stat">
                    ...
                    <strong>{{
                      request.content_diff.baseline_version ||
                      request.version_diff.from_version ||
                      'Unreleased'
                    }}</strong>
                  </div>
                    ...
                    <strong
                      :class="{
                        'c-changed':
                          ...
                          request.content_diff.step_count_before,
                      }"
                    >
                      {{ request.content_diff.step_count_after }}
                    </strong>
                    ...
                    <strong
                      :class="{
                        'c-changed':
                          ...
                          request.content_diff.group_count_before,
                      }"
                    >
                      {{ request.content_diff.group_count_after }}
                    </strong>
                ...
                <div
                  v-if="
                    request.content_diff.input_schema_changed ||
                    request.content_diff.output_schema_changed ||
                    request.content_diff.tags_changed ||
                    request.content_diff.description_changed
                  "
                  class="diff-flags"
                >
                  <t-tag
                    v-if="request.content_diff.input_schema_changed"
                    theme="warning"
                    variant="light"
                    size="small"
                    >{{ t('releaseCenter.diffInputSchemaChanged') }}</t-tag
                  >
                  <t-tag
                    v-if="request.content_diff.output_schema_changed"
                    theme="warning"
                    variant="light"
                    size="small"
                    >{{ t('releaseCenter.diffOutputSchemaChanged') }}</t-tag
                  >
                  <t-tag
                    v-if="request.content_diff.tags_changed"
                    theme="primary"
                    variant="light"
                    size="small"
                    >{{ t('releaseCenter.diffTagsChanged') }}</t-tag
                  >
                  <t-tag
                    v-if="request.content_diff.description_changed"
                    theme="primary"
                    variant="light"
                    size="small"
                    >{{ t('releaseCenter.diffDescriptionChanged') }}</t-tag
                  >
                </div>
              </div>
            </t-card>
            ...
            <template
              v-if="
                (request.content_diff.added_steps?.length ?? 0) > 0 ||
                (request.content_diff.modified_steps?.length ?? 0) > 0 ||
                (request.content_diff.removed_steps?.length ?? 0) > 0
              "
            >
              <div class="diff-columns">
                <t-card :bordered="false">
                  <div class="diff-col-head added">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.added_steps?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.added_steps?.length" class="diff-item-list">
                    <div
                      v-for="item in request.content_diff.added_steps"
                      :key="item.id"
                ...
                <t-card :bordered="false">
                  <div class="diff-col-head modified">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.modified_steps?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.modified_steps?.length" class="diff-item-list">
                    <div
                      v-for="item in request.content_diff.modified_steps"
                      :key="item.id"
                ...
                <t-card :bordered="false">
                  <div class="diff-col-head removed">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.removed_steps?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.removed_steps?.length" class="diff-item-list">
                    <div
                      v-for="item in request.content_diff.removed_steps"
                      :key="item.id"
            ...
            <template
              v-if="
                (request.content_diff.added_groups?.length ?? 0) > 0 ||
                (request.content_diff.modified_groups?.length ?? 0) > 0 ||
                (request.content_diff.removed_groups?.length ?? 0) > 0
              "
            >
              <div class="diff-columns">
                <t-card :bordered="false">
                  <div class="diff-col-head added">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.added_groups?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.added_groups?.length" class="diff-item-list">
                    <div
                      v-for="name in request.content_diff.added_groups"
                      :key="name"
                      class="diff-item diff-item--added"
                    >
                      <strong>{{ name }}</strong>
                ...
                <t-card :bordered="false">
                  <div class="diff-col-head modified">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.modified_groups?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.modified_groups?.length" class="diff-item-list">
                    <div
                      v-for="name in request.content_diff.modified_groups"
                      :key="name"
                      class="diff-item diff-item--modified"
                    >
                      <strong>{{ name }}</strong>
                ...
                <t-card :bordered="false">
                  <div class="diff-col-head removed">
                    ...
                    <span class="diff-col-count">{{
                      request.content_diff.removed_groups?.length ?? 0
                    }}</span>
                  </div>
                  <div v-if="request.content_diff.removed_groups?.length" class="diff-item-list">
                    <div
                      v-for="name in request.content_diff.removed_groups"
                      :key="name"
                      class="diff-item diff-item--removed"
                    >
                      <strong>{{ name }}</strong>
        ...
        <t-tab-panel value="execution" :disabled="!hasExecution">
          <template #label>
            <span class="exec-tab-label">
              ...
              <span
                v-if="executionTabDot"
                class="exec-dot"
                :class="`exec-dot--${executionTabDot}`"
              />
            </span>
          </template>
          ...
          <div class="panel">
            <template v-if="execution">
              <t-card
                v-if="displayExecutionStatus === 'completed'"
                :bordered="false"
                class="banner-card banner-card--success"
              >
              ...
              <t-card
                v-else-if="displayExecutionStatus === 'rollback_in_progress'"
                :bordered="false"
                class="banner-card banner-card--warning"
              >
              ...
              <t-card :bordered="false">
                <div class="exec-stats">
                  <div class="exec-stat">
                    ...
                    <t-tag
                      :theme="instanceTheme(displayExecutionStatus || execution.status)"
                      variant="light"
                      size="small"
                    >
                      {{
                        t(
                          `releaseCenter.executionStatusMap.${
                            displayExecutionStatus || execution.status
                          }`
                        )
                      }}
                  ...
                  <div class="exec-stat">
                    ...
                    <strong class="c-success">{{
                      execution.summary?.succeeded_instances ?? 0
                    }}</strong>
                  </div>
                  <div class="exec-stat">
                ...
                <t-progress
                  ...
                  :percentage="
                    Math.round((execution.current_batch / execution.total_batches) * 100)
                  "
                  :status="
    ...
    >
      ...
      <div class="dialog-actions">
        <t-button variant="outline" @click="reviewDialog.visible = false">{{
          t('common.cancel')
        }}</t-button>
        <t-button
...
/* ── approval ──────────────────────────────────────────────────────────── */
.approval-summary-card :deep(.t-card__body) {
  padding: 16px 20px;
}
.approval-summary {
  display: flex;
...
.my-action-card :deep(.t-card__body) {
  background: rgba(0, 105, 219, 0.04);
  padding: 14px 16px;
}
.my-action {
  display: flex;
...
.approval-row--mine {
  background: rgba(0, 105, 219, 0.03);
  margin: 0 -16px;
  padding: 14px 16px;
  border-radius: 8px;
}
...
.diff-col-head.modified {
  color: #9e7400;
}
.diff-col-head.removed {
  color: #dc503c;
}
.diff-col-sign {
  font-size: 14px;
...
.diff-item--modified {
  background: rgba(158, 116, 0, 0.06);
  border-left-color: #9e7400;
}
.diff-item--removed {
...
/* ── banner cards ───────────────────────────────────────────────────────── */
.banner-card :deep(.t-card__body) {
  padding: 12px 16px;
  font-size: 13px;
  font-weight: 500;
}
.banner-card--success :deep(.t-card__body) {
  color: #00a870;
  background: rgba(0, 168, 112, 0.08);
}
.banner-card--danger :deep(.t-card__body) {
  color: #dc503c;
  background: rgba(220, 80, 60, 0.08);
}
.banner-card--warning :deep(.t-card__body) {
  color: #9e7400;
  background: rgba(235, 167, 0, 0.08);
}
