...
export function applyGroupBasedLayout(
  ...
  for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
    ...
    if (groupIndex > 0 && previousGroupNodeIds.size > 0) {
      nodesInGroup = orderTargetNodesBySourceEdges(nodesInGroup, edges, previousGroupNodeIds);
    }
    ...
    const groupContentWidth =
      depthLevels.length > 0 ? layerX - opts.layerSpacingX - currentX - opts.groupPadding : 200;
...
export function applyDagreLayout(
  ...
  for (const edge of edges) {
    if (
      stepNodes.some((n) => n.id === edge.source) &&
...
export function needsAutoLayout(nodes: FlowNode[]): boolean {
  ...
  const allAtOrigin = stepNodes.every((n) => n.position.x === 0 && n.position.y === 0);
...
function checkForOverlap(nodes: FlowNode[]): boolean {
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      const overlapX =
        Math.abs(a.position.x - b.position.x) < Math.min(aDim.width, bDim.width) * 0.5;
      const overlapY =
        Math.abs(a.position.y - b.position.y) < Math.min(aDim.height, bDim.height) * 0.5;
